 ...
 * @param tokens
 */
function pickUpWhere(tokens) {
  var result = [];
  var res = detectLev(tokens, result);
}
 ...
 * @param res
 */
function detectLev(tokens, res) {
  var open = '(';
  var close = ')';
  for (var i=0;i<tokens.length;i++) {
    // if we found OPEN clause - find next CLOSE
    if (tokens[i] == open) {
      var j = i;
      // number of ()
      var all = 1;
      while ((all != 0) && (j < tokens.length)) {
        j++;
        if (tokens[j] == open) all++;
        else if (tokens[j] == close) all--;
      }
      res.push(detectLev(tokens.substr(i, j)))
    }
  }
}

exports.pickUpWhere = pickUpWhere;
 ...
 * @param sep
 */
function recursivePickUp(tokens, sep) {
  tokens.push({text:sep});
  var res = [];
  var pre = 0;
  var lev = 0;
